...
We will test the program using four different testing methods. The first is unit testing, which tests the individual functions in our code. Next we consider integration testing. This tells us


to see how well different components work together. Afterwards we use system testing for testing the different system components. Lastly, acceptance testing is used to verify the clients requirements. The testing framework that we used is pytest\cite{pytest}, which is part of Python.

\subsubsection{Unit Testing}
Unit testing is a type of sotware testing by which individual units of code are tested for correctness. These are automated tests that should pass every time a change is made which does not include 
\todo{make iets zinnigs hiero}
Unit tests test each method of a function separately, checking that the method does what it is supposed to do. If the method would need information from outside the class that information is mocked. This means that instead of using that other class, a fake object is made which returns a fake value. This ensures the tests will never fail due to changes in other classes. We achieved a test coverage of 85\%.
...
Last we used acceptance testing. These tests verify if the software does what the client expects it to do. The tests are therefore executed by the client manually. After the tests the client can say what worked, what did not work, what was missing and what could be improved. This can for example be used to test if the visualisation looks good to the client.\\
