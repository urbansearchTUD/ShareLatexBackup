...
Now that we have extracted the relationships from the documents and web pages we need to store these results. Note that there is no need to store all the documents because we are interested in the relationships which have been extracted and therefore no need for the original document. To store these relationships we have two options.  These are Graph Databases and Traditional relational databases.


Because we want to visualise the network of cities as a graph and are interested in relations between cities we need a database which best suits this. Unsurprisingly, relations are the most important in the graph data model, where this is not true for traditional relational databases. Therefore, we are confident that a graph database is the best choice.

\subsubsection{Comparing Graph Databases}
Now that we've established that we will be using a Graph Database, we need to choose which Graph Database we're going to use. To do this we made a table in which we compare aspects that are important to us of a few of the most popular Graph Databases.\\
